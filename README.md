...
| `downloadUrl` / `selfHostUrl`           | `downloadUrl` → GitHub Releases landing (`…/releases/latest`). Billing Free **Download** dropdown uses platform asset URLs from [`src/lib/desktopDownloads.ts`](./src/lib/desktopDownloads.ts) (`github` + `name`). **Change `selfHostUrl` when cloning** — default [`docs/SELF_HOSTING.md`](https://github.com/mjf1406/vctr/blob/master/docs/SELF_HOSTING.md). See [`docs/electron.md`](./docs/electron.md). |
